...
5. [ ] [proof](./.proofs/019943aa-1a1f-7c94-a5f2-8b7e3d9c6a45.md) that the `e2e` job tears down the sandbox stack after it finishes regardless of pass/fail (e.g. via `if: always()`), leaving no dangling containers on the runner.
6. [ ] [proof](./.proofs/019943aa-1a20-7de5-b6a3-9c8f4e0d7b56.md) that the `e2e` job runs under `timeout-minutes: 60`, documented in the workflow comment as an initial tunable value covering sandbox bootstrap plus 14 sequential Playwright flows against the sandbox stack rather than a permanent constant.
