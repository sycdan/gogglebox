...
async function withRetry(label, op, { attempts = 5, delayMs = 250 } = {}) {
  let lastError;
...
async function safeScroll(label, getLocator, { block = 'start', timeout = 15_000 } = {}) {
...
try {
  ...
  if (/ignore/i.test(flowName)) {
    ...
    if (await pickHeading.count().then((n) => n > 0)) {
      ...
      const everyone = presetChips.filter({ hasText: /^Everyone$/ }).first();
      if (await everyone.count().then((n) => n > 0)) {
        console.log('[proof] ignore-shows: selecting "Everyone" preset');
        await everyone.click();
      } else if (await presetChips.count().then((n) => n > 0)) {
        console.log('[proof] ignore-shows: "Everyone" preset not found; using first preset chip');
        await presetChips.first().click();
      } else {
        console.warn('[proof] ignore-shows: no .preset-row .chip presets found; selecting first viewer card');
        await page.locator('button.viewer-card').first().click();
      }
      const cont = page.getByRole('button', { name: /^Continue$/ });
      await cont.first().click();
      ...
      console.log('[proof] ignore-shows: already in main app (no viewer-selection screen)');
    }
